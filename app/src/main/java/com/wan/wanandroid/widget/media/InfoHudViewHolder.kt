...
class InfoHudViewHolder(context: Context, tableLayout: TableLayout) {
            ...
            when (msg.what) {
                MSG_UPDATE_HUD -> {
                    ...
                    if (mMediaPlayer is IjkMediaPlayer) {
                        ...
                        if (internal != null && internal is IjkMediaPlayer)
                            mp = internal
                    }
                    if (mp == null)
                        return
                    ...
                    this.sendEmptyMessageDelayed(MSG_UPDATE_HUD, 500)
                }
            }
        }
    }
    ...
    private fun setRowValue(id: Int, value: String) {
        var rowView: View? = mRowMap.get(id)
        if (rowView == null) {
    ...
    companion object {
        ...
        private fun formatedSpeed(bytes: Long, elapsed_milli: Long): String {
            ...
            return if (bytes_per_sec >= 1000 * 1000) {
                String.format(Locale.US, "%.2f MB/s", bytes_per_sec / 1000f / 1000f)
        ...
        private fun formatedSize(bytes: Long): String {
            return if (bytes >= 100 * 1000) {
                String.format(Locale.US, "%.2f MB", bytes.toFloat() / 1000f / 1000f)
            } else if (bytes >= 100) {
